...
# -*- coding: utf-8 -*-
r"""
Code Name : run_vectorbt_micro_exit_coverage_batched_v1_0_2.py
Version   : v1.0.2
Path      : C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_micro_exit_coverage_batched_v1_0_2.py
Run       : python C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_micro_exit_coverage_batched_v1_0_2.py --manifest C:\Data\Bot\central_backtest_results\research_coverage_master_v1_0_0\research_coverage_master_manifest.csv --data-root C:\Data\Bot\central_market_data\parquet --feature-root C:\Data\Bot\central_feature_cache --outdir C:\Data\Bot\central_backtest_results\coverage_master_runs_v1_0_0\micro_exit_expansion_batched_v1_0_2\run_0 --phase micro_exit_expansion --portfolio-chunk-size 64 --progress-every-groups 1 --continue-on-error

Changelog
---------
[v1.0.2]
1) Add schema-safe support for logic_strictness=relaxed and aliases.
2) Add deterministic fallback for swing_variant=medium / mid / neutral / both.
3) Reduce RAM pressure by loading only required parquet columns and downcasting to float32.
4) Keep only one timeframe resident in memory at a time.
5) Improve progress math to use processed jobs (DONE + ERROR + resumed) instead of DONE only.
6) Emit explicit bootstrap warnings when ambiguous side is auto-resolved.

Purpose
-------
Compatibility-safe batched coverage runner for the componentized micro-exit manifest.
This runner reads component fields directly, builds a canonical logic key internally,
and evaluates grouped jobs without requiring legacy `logic_variant` in the manifest.

Backtest / Design Notes
-----------------------
- This file prioritizes robustness and resume safety over maximum speed.
- GPU is not the active execution path here; this runner is CPU/RAM oriented.
- M1 can be very large, so parquet reads are column-limited and floats are downcast.
- `swing_variant=medium` is ambiguous in directional systems. To prevent mass failure,
  this version resolves side deterministically using entry-count dominance.
"""
...
def normalize_job_row(row_index: int, row: Dict[str, str], default_symbol: str, default_phase: str) -> Job:
    ...
    strategy_family = sanitize_text(row.get("strategy_family"), "micro_exit_expansion").lower()
...
def read_feature_parquet(path: Path) -> pd.DataFrame:
    preferred_columns = ["time", "adx_14"]
    try:
        df = pd.read_parquet(path, columns=preferred_columns).copy()
    except Exception:
        df = pd.read_parquet(path).copy()
        keep = [c for c in preferred_columns if c in df.columns]
        if keep:
            df = df[keep].copy()
...
def align_feature_series(price_index: pd.Index, feat_df: pd.DataFrame, column: str, default: float = 0.0) -> pd.Series:
    if column in feat_df.columns:
        s = pd.to_numeric(feat_df[column], errors="coerce")
        s = s.reindex(range(len(price_index)))
        return s.fillna(default).astype("float32")
    return pd.Series(np.full(len(price_index), default, dtype=np.float32), index=price_index)
...
def build_directional_signal(
    ...
    if side == "LONG":
        ...
        return (signal & (~prev)).astype(bool)
...
def build_component_entries(
    ...
    if swing_variant in LONG_SWING_ALIASES:
        ...
        return GroupEvaluation(side="LONG", entries=entries, side_resolution="DIRECT_LONG")
    ...
    if swing_variant in SHORT_SWING_ALIASES:
        ...
        return GroupEvaluation(side="SHORT", entries=entries, side_resolution="DIRECT_SHORT")

    if swing_variant in AMBIGUOUS_SWING_ALIASES:
        long_entries = build_directional_signal(close, ema_fast, ema_slow, adx, adx_threshold, pullback_zone_variant, "LONG")
        short_entries = build_directional_signal(close, ema_fast, ema_slow, adx, adx_threshold, pullback_zone_variant, "SHORT")
        long_count = int(long_entries.sum())
        short_count = int(short_entries.sum())
        if long_count >= short_count:
            return GroupEvaluation(
                side="LONG",
                entries=long_entries,
                side_resolution=f"AUTO_FROM_AMBIGUOUS_SWING_{swing_variant}_LONG_DOMINANT_{long_count}_VS_{short_count}",
            )
        return GroupEvaluation(
            side="SHORT",
            entries=short_entries,
            side_resolution=f"AUTO_FROM_AMBIGUOUS_SWING_{swing_variant}_SHORT_DOMINANT_{short_count}_VS_{long_count}",
        )
...
def build_live_progress(
    ...
    processed = overall_completed + overall_failed
    progress_pct = (processed / overall_total * 100.0) if overall_total else 100.0
    rate_per_min = (processed / elapsed_min) if elapsed_min > 0 else 0.0
    remaining = max(overall_total - processed, 0)
...
def main() -> None:
    ...
    tfc: Optional[Dict[str, object]] = None
    ...
    for group_key, group_jobs in grouped.items():
        ...
        try:
            ...
            if active_timeframe != timeframe:
                ...
                adx = align_feature_series(price_df.index, feat_df, "adx_14", default=0.0)
                tfc = {
                    "symbol": symbol,
                    "price_df": price_df,
                    "feat_df": feat_df,
                ...
                write_bootstrap_log(
                    bootstrap_log_path,
                    f"TIMEFRAME_LOAD timeframe={timeframe} price_path={price_path} feature_path={feature_path} rows={len(price_df)} mode=single_timeframe_resident",
                )

            if tfc is None:
                raise RuntimeError("timeframe cache not initialized")
            ...
            entry_series = group_eval.entries
            if group_eval.side_resolution.startswith("AUTO_FROM_AMBIGUOUS"):
                write_bootstrap_log(bootstrap_log_path, f"GROUP_SIDE_AUTO_RESOLVE group={group_key} resolution={group_eval.side_resolution}")
            ...
            timeframe_summary.append(
                {
                    ...
                    "side": side,
                    "side_resolution": group_eval.side_resolution,
    ...
    write_bootstrap_log(
        bootstrap_log_path,
        f"FINISH final_status={final_status} total_jobs={overall_total} completed={overall_completed} failed={overall_failed} skipped_bad_rows={overall_skipped}",
    )
