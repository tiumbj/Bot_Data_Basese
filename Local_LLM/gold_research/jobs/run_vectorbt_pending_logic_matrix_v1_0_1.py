...
# -*- coding: utf-8 -*-
"""
run_vectorbt_pending_logic_matrix_v1_0_1.py
Version: v1.0.1
...
def read_feature_parquet(path: Path, price_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # fallback for pullback flags: derive from price + ema_20 if upstream fileเก่าขาดคอลัมน์
    merged = df.merge(
...
def build_signals(logic_variant: str, price_df: pd.DataFrame, feat_df: pd.DataFrame, ema_fast: pd.Series, ema_slow: pd.Series):
    ...
    pb_long = feat_df["pullback_to_ema20_long"].fillna(False).astype(bool)
    pb_short = feat_df["pullback_to_ema20_short"].fillna(False).astype(bool)
