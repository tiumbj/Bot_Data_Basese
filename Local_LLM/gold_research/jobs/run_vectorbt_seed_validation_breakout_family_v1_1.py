# ==================================================================================================
# FILE: run_vectorbt_seed_validation_breakout_family_v1_1.py
# PATH: C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_seed_validation_breakout_family_v1_1.py
# VERSION: v1.1.1
#
# CHANGELOG:
# - v1.1.1
#   1) Rewrite whole file for production-grade breakout-family validation on M30
#   2) Fix argparse bug: use args.feature_cache instead of invalid args.feature-cache
#   3) Keep breakout family v1.1 rules:
#      - breakout > swing by at least 0.10 ATR
#      - candle body ratio >= 0.45
#      - ADX >= 28
#      - not extended more than 1.00 ATR from EMA20
#   4) Use trailing ATR stop + structure failure exit so winners can run but drawdown stays controlled
#   5) Keep result contract:
#      - vectorbt_breakout_family_v1_1_results.jsonl
#      - vectorbt_breakout_family_v1_1_results.csv
#      - vectorbt_breakout_family_v1_1_top20.csv
#      - vectorbt_breakout_family_v1_1_summary.json
#      - vectorbt_breakout_family_v1_1_recommendation.txt
# ==================================================================================================
...
OUT_RESULTS_JSONL = "vectorbt_breakout_family_v1_1_results.jsonl"
OUT_RESULTS_CSV = "vectorbt_breakout_family_v1_1_results.csv"
OUT_TOP20_CSV = "vectorbt_breakout_family_v1_1_top20.csv"
OUT_SUMMARY_JSON = "vectorbt_breakout_family_v1_1_summary.json"
OUT_RECOMMENDATION_TXT = "vectorbt_breakout_family_v1_1_recommendation.txt"
...
class FeatureArrays:
    def __init__(self, df: pl.DataFrame) -> None:
        ...
        # ป้องกัน RuntimeWarning: invalid value encountered in divide
        ...
        self.long_breakout_base = self._build_long_breakout_base()
        self.short_breakout_base = self._build_short_breakout_base()

    def _build_long_breakout_base(self) -> np.ndarray:
        valid = (~np.isnan(self.swing_high_10)) & (self.atr_14 > 1e-12)
        breakout_distance = self.close - self.swing_high_10
        breakout_ok = breakout_distance >= (0.10 * self.atr_14)
        body_ok = self.body_ratio >= 0.45
        adx_ok = self.adx_14 >= 28.0
        not_extended = (self.close - self.ema_20) <= (1.00 * self.atr_14)
        bullish_body = self.close > self.open
        price_ok = self.close > self.ema_20
        return valid & self.bull_stack & breakout_ok & body_ok & adx_ok & not_extended & bullish_body & price_ok

    def _build_short_breakout_base(self) -> np.ndarray:
        valid = (~np.isnan(self.swing_low_10)) & (self.atr_14 > 1e-12)
        breakout_distance = self.swing_low_10 - self.close
        breakout_ok = breakout_distance >= (0.10 * self.atr_14)
        body_ok = self.body_ratio >= 0.45
        adx_ok = self.adx_14 >= 28.0
        not_extended = (self.ema_20 - self.close) <= (1.00 * self.atr_14)
        bearish_body = self.close < self.open
        price_ok = self.close < self.ema_20
        return valid & self.bear_stack & breakout_ok & body_ok & adx_ok & not_extended & bearish_body & price_ok
        ...
        for idx, line in enumerate(f, start=1):
            ...
            jobs.append(
                JobSpec(
                    ...
                    strategy_family=normalize_key(first_present(raw, ["strategy_family", "strategy", "strategy_id"], "BREAKOUT_FAMILY_V1_1"), "BREAKOUT_FAMILY_V1_1"),
                    entry_logic=normalize_key(first_present(raw, ["entry_logic", "entry", "entry_type"], "BREAKOUT"), "BREAKOUT"),
...
def entry_mask(arr: FeatureArrays, side: str) -> np.ndarray:
    if side == "LONG":
        return arr.long_breakout_base
    if side == "SHORT":
        return arr.short_breakout_base
...
def micro_exit_profile(micro_exit: str) -> Tuple[float, float]:
    """
    return (trail_atr_multiple, failure_atr_multiple)
    """
...
def build_exit_mask(job: JobSpec, arr: FeatureArrays, side: str) -> np.ndarray:
    trail_mult, failure_mult = micro_exit_profile(job.micro_exit)

    if side == "LONG":
        structure_fail = arr.close < (arr.swing_high_10 - failure_mult * arr.atr_14)
        trend_fail = (arr.close < arr.ema_20) & (arr.rsi_14 < 52.0)
        hard_fail = arr.close < (arr.ema_20 - 0.35 * arr.atr_14)
        reversal = arr.bear_stack
        return structure_fail | trend_fail | hard_fail | reversal

    if side == "SHORT":
        structure_fail = arr.close > (arr.swing_low_10 + failure_mult * arr.atr_14)
...
def evaluate_job(job: JobSpec, arr: FeatureArrays, price: pd.Series, sl_stop: pd.Series) -> Dict[str, Any]:
    ...
    score = (
        ...
        + safe_float(best["profit_factor"]) * 25.0
        + safe_float(best["win_rate_pct"]) * 0.10
        - safe_float(best["max_drawdown_pct"]) * 3.0
...
def build_recommendation(df: pl.DataFrame, summary: Dict[str, Any]) -> str:
    ...
    lines.append("PURPOSE: Breakout family v1.1 seed validation recommendation")
    lines.append("")
    ...
    if int(summary["promoted_jobs"]) > 0:
        lines.append("- Breakout family v1.1 has promotable jobs on M30.")
        lines.append("- Promote only rows that pass promotion gate.")
    else:
        lines.append("- Breakout family v1.1 still has no promotable jobs on M30.")
        lines.append("- This family reduced trade density, but edge is still insufficient.")
    lines.append("")
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VectorBT validation for breakout family v1.1")
...
def main() -> None:
    ...
    feature_cache_path = Path(args.feature_cache)   # แก้ bug ตรงนี้
