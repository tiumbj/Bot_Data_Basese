...
"""
ชื่อโค้ด: repair_feature_cache_add_pullback_flags_v1_0_3.py
ที่อยู่ไฟล์: C:\Data\Bot\Local_LLM\gold_research\jobs\repair_feature_cache_add_pullback_flags_v1_0_3.py
คำสั่งรัน:
    python C:\Data\Bot\Local_LLM\gold_research\jobs\repair_feature_cache_add_pullback_flags_v1_0_3.py
เวอร์ชัน: v1.0.3

เป้าหมาย:
- ซ่อม feature cache schema ให้มี pullback_to_ema20_long / pullback_to_ema20_short
- รองรับ parquet ที่ column เป็น string, tuple, MultiIndex, หรือชื่อพิเศษ
- flatten + canonicalize columns แบบบังคับ
- backup ก่อนเขียนทับ
- เขียน summary ราย TF พร้อม raw/flattened/canonical columns
...
def canonicalize_columns(df: pd.DataFrame, required_time: bool = True) -> Tuple[pd.DataFrame, Dict[str, List[str]]]:
    out = bring_index_to_column_if_needed(df)
    out = flatten_columns(out)

    raw_flat_columns = [str(c) for c in out.columns]
    ...
    for col in out.columns:
        ...
        if norm in TIME_ALIASES:
            canonical = "time"
        elif norm in OPEN_ALIASES:
            canonical = "open"
        elif norm in HIGH_ALIASES:
            canonical = "high"
        elif norm in LOW_ALIASES:
            canonical = "low"
        elif norm in CLOSE_ALIASES:
            canonical = "close"
        elif norm in EMA20_ALIASES:
            canonical = "ema_20"

        if canonical in seen:
            seen[canonical] += 1
            canonical = f"{canonical}__dup{seen[canonical]}"
        else:
            seen[canonical] = 0
    ...
    meta = {
        "raw_flat_columns": raw_flat_columns,
        "canonical_columns": [str(c) for c in out.columns],
    }

    if required_time and "time" not in out.columns:
        raise KeyError(
            f"missing time-like column after canonicalize; available={list(out.columns)}"
        )
...
def normalize_price_columns(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, List[str]]]:
    out, meta = canonicalize_columns(df, required_time=True)

    missing = [c for c in ["high", "low", "close"] if c not in out.columns]
    if missing:
        raise KeyError(
            f"price parquet missing columns={missing}; available={list(out.columns)}"
        )
...
def normalize_feature_columns(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, List[str]]]:
    out, meta = canonicalize_columns(df, required_time=True)

    if "ema_20" not in out.columns:
        raise KeyError(
            f"feature parquet missing 'ema_20'; available={list(out.columns)}"
        )
...
def process_timeframe(
    ...
    row["raw_feature_columns"] = [str(c) for c in raw_feat.columns]
    row["raw_price_columns"] = [str(c) for c in raw_price.columns]

    feat_df, feat_meta = normalize_feature_columns(raw_feat)
    price_df, price_meta = normalize_price_columns(raw_price)

    row["feature_flat_columns"] = feat_meta["raw_flat_columns"]
    row["feature_canonical_columns"] = feat_meta["canonical_columns"]
    row["price_flat_columns"] = price_meta["raw_flat_columns"]
    row["price_canonical_columns"] = price_meta["canonical_columns"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--backup-root",
        type=Path,
        default=Path(r"C:\Data\Bot\central_feature_cache_backups\repair_pullback_flags_v1_0_3"),
        help="Backup directory before overwrite",
    )
    parser.add_argument(
        "--report-root",
        type=Path,
        default=Path(r"C:\Data\Bot\central_backtest_results\repair_feature_cache_add_pullback_flags_v1_0_3"),
