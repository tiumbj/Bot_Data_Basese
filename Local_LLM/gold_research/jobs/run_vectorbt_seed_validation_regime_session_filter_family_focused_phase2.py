# ==================================================================================================
# FILE: run_vectorbt_seed_validation_regime_session_filter_family_focused_phase2.py
# PATH: C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_seed_validation_regime_session_filter_family_focused_phase2.py
# VERSION: v1.0.0
#
# CHANGELOG:
# - v1.0.0
#   1) New production-grade focused phase-2 validation runner
#   2) Read parameter values directly from winner_cluster_manifest.jsonl
#   3) Stop using hash-based profile generation from discovery phase
#   4) Use direct manifest fields:
#      - session_filter
#      - volatility_filter
...
#      - max_extension_atr
#      - cooldown_bars
#   5) Keep proven regime/session filter family architecture:
#      - trend-aligned breakout baseline
#      - regime/session filters
#      - ATR trailing stop
#      - trend failure exit
#      - structure failure exit
#   6) Output:
#      - vectorbt_regime_session_filter_family_focused_phase2_results.jsonl
#      - vectorbt_regime_session_filter_family_focused_phase2_results.csv
#      - vectorbt_regime_session_filter_family_focused_phase2_top20.csv
#      - vectorbt_regime_session_filter_family_focused_phase2_summary.json
#      - vectorbt_regime_session_filter_family_focused_phase2_recommendation.txt
# ==================================================================================================
...
OUT_RESULTS_JSONL = "vectorbt_regime_session_filter_family_focused_phase2_results.jsonl"
OUT_RESULTS_CSV = "vectorbt_regime_session_filter_family_focused_phase2_results.csv"
OUT_TOP20_CSV = "vectorbt_regime_session_filter_family_focused_phase2_top20.csv"
OUT_SUMMARY_JSON = "vectorbt_regime_session_filter_family_focused_phase2_summary.json"
OUT_RECOMMENDATION_TXT = "vectorbt_regime_session_filter_family_focused_phase2_recommendation.txt"
...
def normalize_key(value: Any, default: str = "") -> str:
    return normalize_text(value, default=default).upper().replace("-", "_").replace(" ", "_")
...
class FeatureArrays:
    def __init__(self, df: pl.DataFrame) -> None:
        ...
        # Session masks based on feature-cache bar hour
...
def load_jobs(manifest_path: Path) -> List[JobSpec]:
    ...
    with manifest_path.open("r", encoding="utf-8") as f:
        for idx, line in enumerate(f, start=1):
            ...
            jobs.append(
                JobSpec(
                    job_id=normalize_text(first_present(raw, ["job_id"], f"job_{idx:06d}")),
                    timeframe=normalize_key(first_present(raw, ["timeframe", "tf"], "M30"), "M30"),
                    strategy_family=normalize_key(first_present(raw, ["strategy_family"], "REGIME_SESSION_FILTER_FAMILY_V1_0_0_FOCUSED"), "REGIME_SESSION_FILTER_FAMILY_V1_0_0_FOCUSED"),
                    entry_logic=normalize_key(first_present(raw, ["entry_logic"], "BASE_BREAKOUT_PLUS_FILTER"), "BASE_BREAKOUT_PLUS_FILTER"),
                    regime_filter=normalize_key(first_present(raw, ["regime_filter"], "ALL"), "ALL"),
                    side_policy=normalize_key(first_present(raw, ["side_policy"], "BOTH"), "BOTH"),
                    session_filter=normalize_key(first_present(raw, ["session_filter"], "ALL"), "ALL"),
                    volatility_filter=normalize_key(first_present(raw, ["volatility_filter"], "ALL"), "ALL"),
                    trend_strength_filter=normalize_key(first_present(raw, ["trend_strength_filter"], "ALL"), "ALL"),
                    price_location_filter=normalize_key(first_present(raw, ["price_location_filter"], "ALL"), "ALL"),
                    micro_exit=normalize_key(first_present(raw, ["micro_exit"], "LET_WINNERS_RUN"), "LET_WINNERS_RUN"),
                    adx_min=safe_float(first_present(raw, ["adx_min"], 23.0), 23.0),
                    breakout_atr_min=safe_float(first_present(raw, ["breakout_atr_min"], 0.05), 0.05),
                    body_ratio_min=safe_float(first_present(raw, ["body_ratio_min"], 0.30), 0.30),
                    max_extension_atr=safe_float(first_present(raw, ["max_extension_atr"], 1.00), 1.00),
                    cooldown_bars=safe_int(first_present(raw, ["cooldown_bars"], 0), 0),
                    selected_side_hint=normalize_key(first_present(raw, ["selected_side_hint"], ""), ""),
                    focus_source=normalize_text(first_present(raw, ["focus_source"], ""), ""),
                    focus_rank=safe_int(first_present(raw, ["focus_rank"], idx), idx),
                )
            )
...
def long_base_entry(arr: FeatureArrays, job: JobSpec) -> np.ndarray:
    valid = (~np.isnan(arr.swing_high_10)) & (arr.atr_14 > 1e-12)
    breakout_distance = arr.close - arr.swing_high_10
    breakout_ok = breakout_distance >= (job.breakout_atr_min * arr.atr_14)
    body_ok = arr.body_ratio >= job.body_ratio_min
    trend_ok = arr.bull_stack & (arr.close > arr.ema_20)
    adx_ok = arr.adx_14 >= job.adx_min
    rsi_ok = (arr.rsi_14 >= 48.0) & (arr.rsi_14 <= 72.0)
    not_extended = (arr.close - arr.ema_20) <= (job.max_extension_atr * arr.atr_14)
    impulse_ok = arr.return_1 > -0.0020
    return valid & breakout_ok & body_ok & trend_ok & adx_ok & rsi_ok & not_extended & impulse_ok


def short_base_entry(arr: FeatureArrays, job: JobSpec) -> np.ndarray:
    ...
    adx_ok = arr.adx_14 >= job.adx_min
    rsi_ok = (arr.rsi_14 >= 28.0) & (arr.rsi_14 <= 52.0)
    not_extended = (arr.ema_20 - arr.close) <= (job.max_extension_atr * arr.atr_14)
    impulse_ok = arr.return_1 < 0.0020
    return valid & breakout_ok & body_ok & trend_ok & adx_ok & rsi_ok & not_extended & impulse_ok


def build_side_entries(job: JobSpec, arr: FeatureArrays, side: str) -> np.ndarray:
    base = long_base_entry(arr, job) if side == "LONG" else short_base_entry(arr, job)
    mask = (
        session_mask(arr, job.session_filter)
        & volatility_mask(arr, job.volatility_filter)
        & trend_strength_mask(arr, job.trend_strength_filter)
        & price_location_mask(arr, job.price_location_filter, side)
        & base
    )
...
def evaluate_job(job: JobSpec, arr: FeatureArrays, price: pd.Series) -> Dict[str, Any]:
    ...
    for side in side_list(job.side_policy):
        ...
        exits = build_exit_mask(arr, job.micro_exit, side)
        sl_stop = build_sl_stop(arr, job.micro_exit)
    ...
    row = {
        ...
        "selected_side_hint": job.selected_side_hint,
        ...
        "score": round(score, 6),
        "session_filter": job.session_filter,
        "volatility_filter": job.volatility_filter,
        "trend_strength_filter": job.trend_strength_filter,
        "price_location_filter": job.price_location_filter,
        "side_policy": job.side_policy,
        "micro_exit": job.micro_exit,
        "adx_min": round(job.adx_min, 6),
        "breakout_atr_min": round(job.breakout_atr_min, 6),
        "body_ratio_min": round(job.body_ratio_min, 6),
        "max_extension_atr": round(job.max_extension_atr, 6),
        "cooldown_bars": int(job.cooldown_bars),
        "focus_source": job.focus_source,
        "focus_rank": int(job.focus_rank),
...
def build_recommendation(df: pl.DataFrame, summary: Dict[str, Any]) -> str:
    ...
    lines.append("PURPOSE: Focused phase-2 validation recommendation")
    lines.append("")
    ...
    lines.append(f"PROMOTED_RATE_PCT: {summary['promoted_rate_pct']}")
    lines.append(f"BEST_JOB_ID: {summary['best_job_id']}")
    lines.append("")
    lines.append("TOP 10 JOBS:")

    top_rows = df.sort(["score", "profit_factor", "total_return_pct"], descending=[True, True, True]).head(10).to_dicts()
    ...
    if int(summary["promoted_jobs"]) > 0:
        lines.append("- Focused phase-2 validation preserved a promotable edge.")
        lines.append("- Compare promoted_rate and mean metrics against discovery phase immediately.")
    else:
        lines.append("- Focused phase-2 validation lost the promotable edge.")
        lines.append("- Winner cluster may be too narrow or overfit.")
    lines.append("")
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VectorBT focused phase-2 validation for regime/session filter family")
    parser.add_argument("--manifest", required=True, help="Focused phase-2 manifest JSONL")
...
def main() -> None:
    ...
    df.sort(["score", "profit_factor", "total_return_pct"], descending=[True, True, True]).head(20).write_csv(out_top20)
    ...
    best_job_id = str(df.sort(["score", "profit_factor", "total_return_pct"], descending=[True, True, True]).select("job_id").item(0, 0)) if total_jobs > 0 else ""
    promoted_rate_pct = round((promoted_jobs / total_jobs) * 100.0, 6) if total_jobs > 0 else 0.0
    ...
    summary = {
        ...
        "promoted_rate_pct": promoted_rate_pct,
    ...
    print(f"[DONE] promoted_rate_pct={summary['promoted_rate_pct']}")
