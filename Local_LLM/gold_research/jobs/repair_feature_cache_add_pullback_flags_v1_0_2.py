...
"""
ชื่อโค้ด: repair_feature_cache_add_pullback_flags_v1_0_2.py
ที่อยู่ไฟล์: C:\Data\Bot\Local_LLM\gold_research\jobs\repair_feature_cache_add_pullback_flags_v1_0_2.py
คำสั่งรัน:
    python C:\Data\Bot\Local_LLM\gold_research\jobs\repair_feature_cache_add_pullback_flags_v1_0_2.py
เวอร์ชัน: v1.0.2

เป้าหมาย:
- ซ่อม feature cache schema ให้มี pullback_to_ema20_long / pullback_to_ema20_short
- รองรับ price parquet ที่ใช้ชื่อคอลัมน์ได้หลายแบบ เช่น
  Open/High/Low/Close, open/high/low/close, <HIGH>, etc.
- รองรับเวลาอยู่ใน column หรือ index
- backup ก่อนเขียนทับ
- เขียน summary ที่บอก root cause ราย TF
...
OPEN_CANDIDATES = ["open", "<open>", "open_price", "o"]
HIGH_CANDIDATES = ["high", "<high>", "high_price", "h"]
LOW_CANDIDATES = ["low", "<low>", "low_price", "l"]
CLOSE_CANDIDATES = ["close", "<close>", "close_price", "c"]
...
def bring_index_to_column_if_needed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if any(c in TIME_CANDIDATES for c in normalized_cols):
        return out
...
def rename_by_candidates(df: pd.DataFrame, canonical: str, candidates: List[str]) -> pd.DataFrame:
    out = df.copy()
    rename_map: Dict[object, str] = {}
    for col in out.columns:
        norm = normalize_name(col)
        if norm in candidates and canonical not in out.columns:
...
def normalize_time_column(df: pd.DataFrame, source_name: str) -> pd.DataFrame:
    ...
    time_rename_map: Dict[object, str] = {}
    for col in out.columns:
        if normalize_name(col) in TIME_CANDIDATES:
            time_rename_map[col] = "time"
            break
    if time_rename_map:
        out = out.rename(columns=time_rename_map)
...
def normalize_price_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # rename OHLC variants to canonical names
    out = rename_by_candidates(out, "open", OPEN_CANDIDATES)
    out = rename_by_candidates(out, "high", HIGH_CANDIDATES)
    out = rename_by_candidates(out, "low", LOW_CANDIDATES)
    out = rename_by_candidates(out, "close", CLOSE_CANDIDATES)

    missing = [c for c in ["high", "low", "close"] if c not in out.columns]
    if missing:
        normalized_cols = [normalize_name(c) for c in out.columns]
        raise KeyError(
            f"price parquet missing columns={missing}; available_columns={list(out.columns)}; normalized={normalized_cols}"
        )
...
def normalize_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # map ema_20 variants if needed
    if "ema_20" not in out.columns:
        for col in out.columns:
            norm = normalize_name(col)
            if norm in {"ema_20", "ema20"}:
                out = out.rename(columns={col: "ema_20"})
                break

    if "ema_20" not in out.columns:
        normalized_cols = [normalize_name(c) for c in out.columns]
        raise KeyError(
            f"feature parquet missing 'ema_20'; available_columns={list(out.columns)}; normalized={normalized_cols}"
        )
...
def process_timeframe(
    ...
    row["normalized_feature_columns"] = [str(c) for c in feat_df.columns]
    row["normalized_price_columns"] = [str(c) for c in price_df.columns]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--backup-root",
        type=Path,
        default=Path(r"C:\Data\Bot\central_feature_cache_backups\repair_pullback_flags_v1_0_2"),
        help="Backup directory before overwrite",
    )
    parser.add_argument(
        "--report-root",
        type=Path,
        default=Path(r"C:\Data\Bot\central_backtest_results\repair_feature_cache_add_pullback_flags_v1_0_2"),
