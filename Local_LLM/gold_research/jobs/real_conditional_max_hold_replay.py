# version: v1.0.0
# file: C:\Data\Bot\Local_LLM\gold_research\jobs\real_conditional_max_hold_replay.py
# run:
# python C:\Data\Bot\Local_LLM\gold_research\jobs\real_conditional_max_hold_replay.py --trades C:\Data\Bot\central_backtest_results\paper_trade_historical\trades.jsonl --ohlc "C:\Data\Bot\central_market_data\tf\XAUUSD_M30.csv" --outdir C:\Data\Bot\central_backtest_results\real_conditional_max_hold_replay --time-column datetime --close-column close
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Real replay for regime block + conditional max hold exits")
...
def build_effective_row_conditional_max_hold(
    ...
    # conditional rule:
    # close only if pnl ณ threshold <= 0
    if threshold_pnl <= 0:
...
def summarize_variant(name: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    conditional_exit_count = sum(1 for r in rows if r["effective_exit_reason"] == "conditional_max_hold_exit")
...
def main() -> None:
    ...
    baseline_rows = [base_effective_row(row) for row in filtered_trades]
    conditional_6h_rows = [
        build_effective_row_conditional_max_hold(row, 6.0, ts_index, bars)
    ...
    conditional_6h_summary = summarize_variant("conditional_max_hold_6h_if_pnl_not_positive", conditional_6h_rows)
    conditional_4h_summary = summarize_variant("conditional_max_hold_4h_if_pnl_not_positive", conditional_4h_rows)

    write_jsonl(outdir / "baseline_filtered.jsonl", baseline_rows)
    write_jsonl(outdir / "conditional_max_hold_6h_if_pnl_not_positive.jsonl", conditional_6h_rows)
    write_jsonl(outdir / "conditional_max_hold_4h_if_pnl_not_positive.jsonl", conditional_4h_rows)

    with (outdir / "variant_results.jsonl").open("w", encoding="utf-8") as f:
        for row in [baseline_summary, conditional_6h_summary, conditional_4h_summary]:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
    ...
    report = {
        ...
        "variants": [
            baseline_summary,
            conditional_6h_summary,
            conditional_4h_summary,
        ],
    }
