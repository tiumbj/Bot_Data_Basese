# ==================================================================================================
# FILE: run_vectorbt_seed_validation_breakout_family_v1_2.py
# PATH: C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_seed_validation_breakout_family_v1_2.py
# VERSION: v1.2.1
#
# CHANGELOG:
# - v1.2.1
#   1) Rewrite whole file again for production-grade breakout-family validation on M30
#   2) Keep argparse bug fixed: use args.feature_cache
#   3) Fix NameError in portfolio_metrics: use max_drawdown_pct correctly
#   4) Keep relaxed breakout family v1.2 entry logic:
#      - breakout > swing by at least 0.05 ATR
#      - candle body ratio >= 0.35
#      - ADX >= 23
#      - not extended more than 1.25 ATR from EMA20
#      - momentum guard from return_1
#   5) Keep let-winners-run exit architecture:
#      - ATR trailing stop
#      - structure failure exit
#      - trend failure exit
#   6) Keep result contract:
#      - vectorbt_breakout_family_v1_2_results.jsonl
#      - vectorbt_breakout_family_v1_2_results.csv
#      - vectorbt_breakout_family_v1_2_top20.csv
#      - vectorbt_breakout_family_v1_2_summary.json
#      - vectorbt_breakout_family_v1_2_recommendation.txt
# ==================================================================================================
...
OUT_RESULTS_JSONL = "vectorbt_breakout_family_v1_2_results.jsonl"
OUT_RESULTS_CSV = "vectorbt_breakout_family_v1_2_results.csv"
OUT_TOP20_CSV = "vectorbt_breakout_family_v1_2_top20.csv"
OUT_SUMMARY_JSON = "vectorbt_breakout_family_v1_2_summary.json"
OUT_RECOMMENDATION_TXT = "vectorbt_breakout_family_v1_2_recommendation.txt"
...
class FeatureArrays:
    ...
    def _build_long_breakout_base(self) -> np.ndarray:
        ...
        breakout_ok = breakout_distance >= (0.05 * self.atr_14)
        body_ok = self.body_ratio >= 0.35
        adx_ok = self.adx_14 >= 23.0
        not_extended = (self.close - self.ema_20) <= (1.25 * self.atr_14)
        bullish_body = self.close > self.open
        price_ok = self.close > self.ema_20
        momentum_ok = self.return_1 > 0.0
    ...
    def _build_short_breakout_base(self) -> np.ndarray:
        ...
        body_ok = self.body_ratio >= 0.35
        adx_ok = self.adx_14 >= 23.0
        not_extended = (self.ema_20 - self.close) <= (1.25 * self.atr_14)
        ...
        for idx, line in enumerate(f, start=1):
            ...
            jobs.append(
                JobSpec(
                    ...
                    strategy_family=normalize_key(first_present(raw, ["strategy_family", "strategy", "strategy_id"], "BREAKOUT_FAMILY_V1_2"), "BREAKOUT_FAMILY_V1_2"),
...
def build_recommendation(df: pl.DataFrame, summary: Dict[str, Any]) -> str:
    ...
    lines.append("PURPOSE: Breakout family v1.2 seed validation recommendation")
    lines.append("")
    ...
    if int(summary["promoted_jobs"]) > 0:
        lines.append("- Breakout family v1.2 has promotable jobs on M30.")
        lines.append("- Promote only rows that pass promotion gate.")
    else:
        lines.append("- Breakout family v1.2 still has no promotable jobs on M30.")
        lines.append("- If trade_count rises but PF stays weak, this family still lacks edge.")
    lines.append("")
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VectorBT validation for breakout family v1.2")
