...
# -*- coding: utf-8 -*-
"""
run_vectorbt_pending_logic_matrix_v1_0_2.py
Version: v1.0.2

Use top EMA pairs from uncovered matrix and expand pending logic families.
Production fixes:
- robust parquet schema normalization
- feature schema validation
- feature-root default fixed to central_feature_cache
- safe fallback derive pullback flags from price + ema_20 if missing
- clearer MISSING_INPUT / SCHEMA_ERROR states
- remove pandas FutureWarning from shift().fillna(False).astype(bool)
"""
...
def canonicalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in out.columns:
        ...
        elif norm in EMA20_ALIASES and "ema_20" not in out.columns:
...
def read_price_parquet(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"missing required price columns in {path}: {missing}; available={list(df.columns)}")
...
def read_feature_parquet(path: Path, price_df: pd.DataFrame) -> pd.DataFrame:
    df = pd.read_parquet(path)
    df = canonicalize_columns(df)

    if "time" not in df.columns:
        raise ValueError(f"missing feature time column in {path}; available={list(df.columns)}")

    df["time"] = pd.to_datetime(df["time"], errors="coerce")
    df = df.dropna(subset=["time"]).sort_values("time").drop_duplicates(subset=["time"], keep="last").reset_index(drop=True)

    required_feature_cols = ["adx_14", "ema_20", "trend_bucket"]
    missing_required = [c for c in required_feature_cols if c not in df.columns]
    if missing_required:
        raise ValueError(f"missing required feature columns in {path}: {missing_required}; available={list(df.columns)}")
    ...
    if "pullback_to_ema20_long" not in merged.columns:
        merged["pullback_to_ema20_long"] = ((merged["low"] <= merged["ema_20"]) & (merged["close"] >= merged["ema_20"])).astype(np.int8)

    if "pullback_to_ema20_short" not in merged.columns:
        merged["pullback_to_ema20_short"] = ((merged["high"] >= merged["ema_20"]) & (merged["close"] <= merged["ema_20"])).astype(np.int8)

    merged["adx_14"] = pd.to_numeric(merged["adx_14"], errors="coerce").fillna(0.0)
    merged["ema_20"] = pd.to_numeric(merged["ema_20"], errors="coerce")
    merged["pullback_to_ema20_long"] = merged["pullback_to_ema20_long"].fillna(0).astype(bool)
    merged["pullback_to_ema20_short"] = merged["pullback_to_ema20_short"].fillna(0).astype(bool)

    if merged["ema_20"].isna().any():
        raise ValueError(f"feature ema_20 contains NaN after normalization in {path}")
...
def as_bool_series(series: pd.Series, index: pd.Index) -> pd.Series:
...
def shifted_prev_bool(series: pd.Series, index: pd.Index) -> pd.Series:
    s = as_bool_series(series, index)
    return s.shift(1, fill_value=False).astype(bool)
...
def build_signals(logic_variant: str, price_df: pd.DataFrame, feat_df: pd.DataFrame, ema_fast: pd.Series, ema_slow: pd.Series):
    ...
    adx = pd.to_numeric(feat_df["adx_14"], errors="coerce").fillna(0.0)
    pb_long = as_bool_series(feat_df["pullback_to_ema20_long"], idx)
    pb_short = as_bool_series(feat_df["pullback_to_ema20_short"], idx)

    fast_gt_slow = as_bool_series(ema_fast > ema_slow, idx)
    fast_lt_slow = as_bool_series(ema_fast < ema_slow, idx)
    adx_ge_25 = as_bool_series(adx >= 25.0, idx)
    adx_lt_20 = as_bool_series(adx < 20.0, idx)
    close_lt_slow = as_bool_series(close < ema_slow, idx)
    close_gt_slow = as_bool_series(close > ema_slow, idx)

    if logic_variant == "ema_cross_long_only":
        long_signal = fast_gt_slow
        prev = shifted_prev_bool(long_signal, idx)
        entries = long_signal & (~prev)
        exits = (~long_signal) & prev
        return entries, exits, None, None

    if logic_variant == "ema_cross_short_only":
        short_signal = fast_lt_slow
        prev = shifted_prev_bool(short_signal, idx)
        short_entries = short_signal & (~prev)
        short_exits = (~short_signal) & prev
        return pd.Series(False, index=idx), pd.Series(False, index=idx), short_entries, short_exits

    if logic_variant == "ema_pullback_long_only":
        base = fast_gt_slow & pb_long
        prev = shifted_prev_bool(base, idx)
        entries = base & (~prev)
        exits = close_lt_slow | ((~base) & prev)
        return entries, exits, None, None

    if logic_variant == "ema_pullback_short_only":
        base = fast_lt_slow & pb_short
        prev = shifted_prev_bool(base, idx)
        short_entries = base & (~prev)
        short_exits = close_gt_slow | ((~base) & prev)
        return pd.Series(False, index=idx), pd.Series(False, index=idx), short_entries, short_exits

    if logic_variant == "adx_ema_cross_long_only":
        base = fast_gt_slow & adx_ge_25
        prev = shifted_prev_bool(base, idx)
        entries = base & (~prev)
        exits = (~fast_gt_slow) | adx_lt_20
        return entries, exits, None, None

    if logic_variant == "adx_ema_cross_short_only":
        base = fast_lt_slow & adx_ge_25
        prev = shifted_prev_bool(base, idx)
        short_entries = base & (~prev)
        short_exits = (~fast_lt_slow) | adx_lt_20
        return pd.Series(False, index=idx), pd.Series(False, index=idx), short_entries, short_exits
...
def main() -> None:
    ...
    parser.add_argument("--feature-root", type=Path, default=Path(r"C:\Data\Bot\central_feature_cache"))
    ...
    for tf in timeframes:
        ...
        try:
            ...
            if tf not in cache_feat:
                cache_feat[tf] = read_feature_parquet(feature_path, cache_price[tf])
            ...
            summary_rows.append({"timeframe": tf, "status": "SCHEMA_ERROR", "error": str(exc)})
            print(f"[SCHEMA_ERROR] timeframe={tf} error={exc}")
            continue
        ...
        for batch_start in range(0, len(tf_jobs), args.batch_size):
            ...
            print(
                f"[PROGRESS] timeframe={tf} batch={(batch_start // args.batch_size)+1} "
                f"tf_completed={tf_done}/{len(tf_jobs)} "
                f"tf_progress_pct={(tf_done / len(tf_jobs) * 100.0) if tf_jobs else 100.0:.2f} "
