...
# -*- coding: utf-8 -*-
r"""
Code Name : run_vectorbt_micro_exit_coverage_batched_v1_0_8.py
Version   : v1.0.8
Path      : C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_micro_exit_coverage_batched_v1_0_8.py
Run       : python C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_micro_exit_coverage_batched_v1_0_8.py --manifest C:\Data\Bot\central_backtest_results\research_coverage_master_v1_0_0\research_coverage_master_manifest.csv --data-root C:\Data\Bot\central_market_data\parquet --feature-root C:\Data\Bot\central_feature_cache --outdir C:\Data\Bot\central_backtest_results\coverage_master_runs_v1_0_0\micro_exit_expansion_batched_v1_0_8\run_v108_full --phase micro_exit_expansion --portfolio-chunk-size 16 --preflight-flush-size 5000 --progress-every-groups 1 --continue-on-error

Purpose
- Coverage runner แบบ component-schema สำหรับ micro-exit expansion
- ฐานจากแนว execution ของ v1.0.6 และขยาย support ให้ market_structure_continuation และปลด block logic_strictness=relaxed / swing_variant=medium
- ตีความ logic_strictness=relaxed เป็น threshold ADX ต่ำสุดของกลุ่ม trend filter
- ตีความ swing_variant=medium เป็น dual-side execution (long+short) ใน job เดียว
- เพิ่ม shard filter เพื่อยิงหลาย process ขนานบน CPU ได้ทันที
...
SUPPORTED_STRATEGY_FAMILIES = {
    ...
    "bos_continuation",
    "swing_pullback",
    "market_structure_continuation",  # added in v1.0.8
}

SUPPORTED_ENTRY_VARIANTS = {"confirm_entry"}
SUPPORTED_PULLBACK_ZONE_VARIANTS = {"narrow", "medium", "wide"}
SUPPORTED_SWING_VARIANTS = {"long", "short", "medium"}  # medium => dual-side execution in v1.0.8
...
def normalize_timeframe_filter(value: str) -> List[str]:
    if not value:
        return []
    items = [x.strip().upper() for x in str(value).split(",") if x.strip()]
    return [x for x in items if x in TIMEFRAMES]


def row_matches_runtime_filters(
    row_index: int,
    row: dict,
    shard_index: int,
    shard_count: int,
    timeframe_filter: List[str],
) -> Tuple[bool, str]:
    timeframe = normalize_text(row.get("timeframe")).upper()
    if timeframe_filter and timeframe not in timeframe_filter:
        return False, f"filtered timeframe={timeframe}"
    if shard_count > 1 and (row_index % shard_count) != shard_index:
        return False, f"filtered shard={shard_index}/{shard_count}"
    return True, ""
...
def compile_job_spec(row: dict) -> Tuple[Optional[JobSpec], str, str]:
    ...
    if logic_strictness not in {"relaxed", "loose", "medium", "strict"}:
...
def preflight_manifest(
    ...
    with manifest_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row_index, row in enumerate(reader):
            ...
            runtime_ok, runtime_reason = row_matches_runtime_filters(row_index, row, shard_index, shard_count, timeframe_filter)
            if not runtime_ok:
                spec, status, reason = None, "SKIPPED_ROW", runtime_reason
            else:
...
def build_entries_bos_continuation(job: JobSpec, df: pd.DataFrame, trend: pd.DataFrame, pb: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    ...
    long_entry = trend["trend_up"] & bos_long.shift(1).astype("boolean").fillna(False).astype(bool) & pb["long_pullback"] & pb["long_confirm"]
    short_entry = trend["trend_down"] & bos_short.shift(1).astype("boolean").fillna(False).astype(bool) & pb["short_pullback"] & pb["short_confirm"]
    return long_entry.fillna(False), short_entry.fillna(False)
...
def build_entries_market_structure_continuation(job: JobSpec, df: pd.DataFrame, trend: pd.DataFrame, pb: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    lookback = max(10, min(job.ema_slow // 2, 40))
    rolling_high = df["high"].rolling(lookback).max().shift(2)
    rolling_low = df["low"].rolling(lookback).min().shift(2)

    bullish_break = (df["close"].shift(1) > rolling_high) & (df["close"] > trend["ema_fast"])
    bearish_break = (df["close"].shift(1) < rolling_low) & (df["close"] < trend["ema_fast"])

    long_body_confirm = (df["close"] > df["open"]) & (df["close"] >= df["high"].rolling(3).max().shift(1).fillna(df["close"]))
    short_body_confirm = (df["close"] < df["open"]) & (df["close"] <= df["low"].rolling(3).min().shift(1).fillna(df["close"]))

    long_entry = trend["trend_up"] & bullish_break & pb["long_pullback"] & pb["long_confirm"] & long_body_confirm
    short_entry = trend["trend_down"] & bearish_break & pb["short_pullback"] & pb["short_confirm"] & short_body_confirm
    return long_entry.fillna(False), short_entry.fillna(False)
...
def build_entries_for_component_job(job: JobSpec, df: pd.DataFrame) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    elif job.strategy_family == "market_structure_continuation":
        long_entry, short_entry = build_entries_market_structure_continuation(job, df, trend, pb)
    else:
...
def combine_metrics(metrics_a: dict, metrics_b: dict) -> dict:
    trade_count = int(metrics_a["trade_count"]) + int(metrics_b["trade_count"])
    wins = int(metrics_a["wins"]) + int(metrics_b["wins"])
    losses = int(metrics_a["losses"]) + int(metrics_b["losses"])
    pnl_sum = round(float(metrics_a["pnl_sum"]) + float(metrics_b["pnl_sum"]), 6)
    avg_pnl = round((pnl_sum / trade_count), 6) if trade_count else 0.0
    win_rate_pct = round((wins / trade_count) * 100.0, 4) if trade_count else 0.0

    gp_a = max(float(metrics_a["pnl_sum"]), 0.0)
    gp_b = max(float(metrics_b["pnl_sum"]), 0.0)
    gl_a = abs(min(float(metrics_a["pnl_sum"]), 0.0))
    gl_b = abs(min(float(metrics_b["pnl_sum"]), 0.0))
    gross_profit = gp_a + gp_b
    gross_loss = gl_a + gl_b
    profit_factor = round((gross_profit / gross_loss) if gross_loss > 0 else (999.0 if gross_profit > 0 else 0.0), 6)
        ...
        for gk, group_jobs in grouped.items():
            ...
            try:
                ...
                for job in group_jobs:
                    if job.swing_variant == "long":
                        metrics = simulate_side(df, trend, long_entry, "LONG", job.micro_exit_variant)
                    elif job.swing_variant == "short":
                        metrics = simulate_side(df, trend, short_entry, "SHORT", job.micro_exit_variant)
                    else:
                        long_metrics = simulate_side(df, trend, long_entry, "LONG", job.micro_exit_variant)
                        short_metrics = simulate_side(df, trend, short_entry, "SHORT", job.micro_exit_variant)
                        metrics = combine_metrics(long_metrics, short_metrics)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeframe-filter", default="")
...
def main() -> None:
    ...
    timeframe_filter = normalize_timeframe_filter(args.timeframe_filter)
    ...
    logger.log(
        f"RUNTIME_FILTERS shard_index={args.shard_index} shard_count={args.shard_count} timeframe_filter={','.join(timeframe_filter) if timeframe_filter else 'ALL'}"
    )
