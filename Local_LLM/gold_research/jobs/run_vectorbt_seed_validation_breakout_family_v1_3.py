# ==================================================================================================
# FILE: run_vectorbt_seed_validation_breakout_family_v1_3.py
# PATH: C:\Data\Bot\Local_LLM\gold_research\jobs\run_vectorbt_seed_validation_breakout_family_v1_3.py
# VERSION: v1.3.0
#
# CHANGELOG:
# - v1.3.0
#   1) Rewrite whole file for production-grade breakout-family validation on M30
#   2) Keep argparse bug fixed: use args.feature_cache
#   3) Change entry structure from instant breakout to breakout + retest confirmation
#   4) Keep let-winners-run exit architecture with ATR trailing stop + structure failure
#   5) Keep output contract:
#      - vectorbt_breakout_family_v1_3_results.jsonl
#      - vectorbt_breakout_family_v1_3_results.csv
#      - vectorbt_breakout_family_v1_3_top20.csv
#      - vectorbt_breakout_family_v1_3_summary.json
#      - vectorbt_breakout_family_v1_3_recommendation.txt
#
# ENTRY LOGIC (v1.3.0):
# - Long:
#   - close > swing_high_10
#   - low retests breakout zone within max(0.25 ATR, 0.06% of price)
#   - bull stack + above ema stack
#   - ADX >= 26
#   - MID/HIGH VOL only
#   - return_1 > 0.10%
#   - RSI 54 to 72
#   - not extended above EMA20 by more than 1.20 ATR
#
# - Short:
#   - close < swing_low_10
#   - high retests breakout zone within max(0.25 ATR, 0.06% of price)
#   - bear stack + below ema stack
#   - ADX >= 26
#   - MID/HIGH VOL only
#   - return_1 < -0.10%
#   - RSI 28 to 46
#   - not extended below EMA20 by more than 1.20 ATR
# ==================================================================================================
...
OUT_RESULTS_JSONL = "vectorbt_breakout_family_v1_3_results.jsonl"
OUT_RESULTS_CSV = "vectorbt_breakout_family_v1_3_results.csv"
OUT_TOP20_CSV = "vectorbt_breakout_family_v1_3_top20.csv"
OUT_SUMMARY_JSON = "vectorbt_breakout_family_v1_3_summary.json"
OUT_RECOMMENDATION_TXT = "vectorbt_breakout_family_v1_3_recommendation.txt"
...
class FeatureArrays:
    ...
    def _build_long_breakout_base(self) -> np.ndarray:
        ...
        retest_ok = self.low <= (self.swing_high_10 + self.retest_band)
        trend_ok = self.bull_stack & self.mask_above_ema
        adx_ok = self.adx_14 >= 26.0
        vol_ok = self.mask_mid_vol | self.mask_high_vol
        impulse_ok = (self.return_1 > 0.0010) & (self.rsi_14 >= 54.0) & (self.rsi_14 <= 72.0)
        not_extended = self.close <= (self.ema_20 + self.atr_14 * 1.20)
        return valid & breakout & retest_ok & trend_ok & adx_ok & vol_ok & impulse_ok & not_extended
    ...
    def _build_short_breakout_base(self) -> np.ndarray:
        ...
        retest_ok = self.high >= (self.swing_low_10 - self.retest_band)
        trend_ok = self.bear_stack & self.mask_below_ema
        adx_ok = self.adx_14 >= 26.0
        vol_ok = self.mask_mid_vol | self.mask_high_vol
            ...
            jobs.append(
                JobSpec(
                    ...
                    strategy_family=normalize_key(
                        first_present(raw, ["strategy_family", "strategy", "strategy_id"], "BREAKOUT_FAMILY_V1_3"),
                        "BREAKOUT_FAMILY_V1_3",
                    ),
                    entry_logic=normalize_key(first_present(raw, ["entry_logic", "entry", "entry_type"], "BREAKOUT_RETEST"), "BREAKOUT_RETEST"),
...
def build_exit_mask(job: JobSpec, arr: FeatureArrays, side: str) -> np.ndarray:
    ...
    if side == "LONG":
        ...
        trend_fail = (arr.close < arr.ema_20) & (arr.rsi_14 < 50.0)
...
def build_recommendation(df: pl.DataFrame, summary: Dict[str, Any]) -> str:
    ...
    lines.append("PURPOSE: Breakout family v1.3 seed validation recommendation")
    lines.append("")
    ...
    if int(summary["promoted_jobs"]) > 0:
        lines.append("- Breakout family v1.3 has promotable jobs on M30.")
        lines.append("- Promote only rows that pass promotion gate.")
    else:
        lines.append("- Breakout family v1.3 still has no promotable jobs on M30.")
        lines.append("- If retest entry still cannot produce PF > 1, breakout family likely lacks edge on this TF.")
    lines.append("")
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VectorBT validation for breakout family v1.3")
