# ============================================================
# ชื่อโค้ด: patch_database_path_references.py
# ที่อยู่ไฟล์: C:\Data\Bot\Local_LLM\gold_research\jobs\patch_database_path_references.py
# คำสั่งรัน:
#   python C:\Data\Bot\Local_LLM\gold_research\jobs\patch_database_path_references.py --dry-run
#   python C:\Data\Bot\Local_LLM\gold_research\jobs\patch_database_path_references.py --execute
# เวอร์ชัน: v1.0.1
# ============================================================
...
REPLACEMENTS: List[Tuple[str, str]] = [
    (
        r"C:\Data\data_base\canonical_dataset\dataset",
        r"C:\Data\data_base\canonical_dataset\dataset",
    ),
    (
        r"C:\Data\data_base\backtest_results\central_backtest_results",
        r"C:\Data\data_base\backtest_results\central_backtest_results",
    ),
    (
        r"C:\Data\data_base\backtest_results\gold_research_results",
        r"C:\Data\data_base\backtest_results\gold_research_results",
    ),
    (
        r"C:\Data\data_base\backtest_results\gold_research_outputs",
        r"C:\Data\data_base\backtest_results\gold_research_outputs",
    ),
    (
        r"C:\Data\data_base\logs\gold_research_logs",
        r"C:\Data\data_base\logs\gold_research_logs",
    ),
]
...
def backup_file(source_path: Path, backup_root: Path, root: Path) -> Path:
    ...
    backup_dir = backup_root / f"patch_database_path_references_{timestamp}"
...
def print_header(root: Path, dry_run: bool) -> None:
    print("=" * 120)
    print(f"[INFO] patch_database_path_references.py version={VERSION}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch old database path references to the new central data_base root."
    )
    parser.add_argument(
        "--root",
    ...
    summary_path = save_summary("patch_database_path_references", summary_payload)
